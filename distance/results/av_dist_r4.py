...
def get_averages(dataset,s):
    ...
    ns = []
    for i in range(100):
        with open("{}/r4_s{}_100/code_{}.out".format(dataset,s,i+1),'r') as f:
            lines = f.readlines()
            try:
...
if __name__ == "__main__":
    ...
    s_vals = np.array([5,7,11,13,17,23])
    datasets = ['r4_absame_nodiffcheck', 'r4_abdiff_nodiffcheck']
    all_av_dists = []
    all_stderrs = []
    all_av_ns = []
    fits = []
    ...
    plt.legend()
    plt.savefig('av_dist_r4.pdf', format='pdf')
    plt.show()
